...
const defaultSettings: TeacherPaymentSettings = {
  plans: {
    oneMonth: {
      price: 19900,
      name: '1개월 이용권',
      benefits: '프로필 노출 및 지원'
    },
    threeMonth: {
      price: 49900,
      originalPrice: 59700,
  ...
  messages: {
    depositInstruction: "입금자명은 반드시 '김00' 전문가님 성함으로 입금해주세요.",
    paymentNotice: '입금 확인은 즉시 자동으로 처리되며, 완료 후 바로 프로필이 활성화 됩니다.'
  }
};
...
export default function TeacherPaymentPage() {
  ...
  const [settings, setSettings] = useState<TeacherPaymentSettings>(defaultSettings);
    ...
    if (!authLoading) {
      ...
      if (userData && userData.userType !== 'therapist') {
        ...
        if (userData.userType === 'parent') {
          // 학부모는 학부모용 결제 페이지로
          alert('학부모 계정입니다. 학부모용 이용권 페이지로 이동합니다.');
  ...
  // Firebase에서 설정 정보 가져오기 (치료사용 설정)
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const settingsDoc = await getDoc(doc(db, 'system_settings', 'teacher_payment_config'));
        
        if (settingsDoc.exists()) {
          setSettings(settingsDoc.data() as TeacherPaymentSettings);
        } else {
          // Firebase에 초기 설정 자동 생성
          await setDoc(doc(db, 'system_settings', 'teacher_payment_config'), defaultSettings);
          setSettings(defaultSettings);
        }
      } catch (error) {
        console.error('치료사 결제 설정 로딩 실패:', error);
        setSettings(defaultSettings);
      } finally {
        setLoading(false);
      }
    ...
    // 인증된 사용자이고 치료사인 경우에만 설정 로딩
    if (!authLoading && currentUser && userData && userData.userType === 'therapist') {
      fetchSettings();
    } else if (!authLoading) {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-white py-8">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">전문가 이용권 결제</h1>
          <p className="text-gray-600">원하시는 이용권을 선택하고 플랫폼 활동을 시작해보세요.</p>
        </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        {plans['3month'].duration} + <span className="text-red-500">16% 할인</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
